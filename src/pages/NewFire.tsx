...
import { ROUTES, STORAGE_KEYS, BACKGROUND_OPTIONS, GUEST_DEFAULT_BACKGROUND, VALIDATION_CONFIG } from '@/constants';
...
export default function NewFire() {
  ...
  const handleProfileClick = () => {
    log.debug('Profile clicked', null, 'NewFire');
    navigate('/profile');
  };
  ...
  return (
    <div 
      ...
      style={{ 
        maxWidth: '100vw',
        backgroundColor: selectedBackground === 'custom' && customBackgroundImage 
          ? undefined 
          : (BACKGROUND_OPTIONS.find(opt => opt.id === selectedBackground)?.bgColor || '#FFE5D9'),
        position: 'relative'
      }}
    >
      {/* 背景圖片層 */}
      ...
      <section aria-labelledby="publish-prayer-heading" className="w-full flex flex-col items-center px-4 relative z-10" style={{ paddingTop: '240px' }}>
        {/* 代禱輸入卡片 */}
        <div className="w-full max-w-[358px] flex justify-center">
          ...
          >
            <form onSubmit={handlePrayerSubmit} className="flex flex-col items-center pt-[15px] pb-[24px] relative z-20">
              <div
                className="w-[300px] h-[300px] border text-sm font-normal px-3 py-2 border-[#1694da] border-solid flex items-start relative"
              >
                <Textarea
                  value={prayerText}
              ...
              {uploadError && <div className="text-red-500 text-xs mt-1 z-20">{uploadError}</div>}
              {uploading && <div className="text-blue-500 text-xs mt-1 z-20">圖片上傳中...</div>}
              <input
                ref={fileInputRef}
                type="file"
              ...
              />
              {/* 更改背景、麥克風、照相機和箭頭圖示群組 */}
              <div className="flex justify-center mt-[24px] relative z-20">
                <div className="flex items-center gap-[60px]">
                  <button 
                    type="button" 
                    onClick={handleBackgroundClick}
                    className="cursor-pointer hover:opacity-70 transition-opacity p-2 z-20"
                    title="更改背景"
                  >
                    <img
                  ...
                  <button 
                    ...
                    className={`cursor-pointer disabled:cursor-not-allowed hover:opacity-80 transition-opacity p-2 z-20 ${
        ...
        <div className="w-full max-w-[358px] flex justify-center" style={{ marginTop: '120px' }}>
          <div
            className="h-[70px] p-4 shadow-sm flex items-center w-[330px] rounded-none relative z-10"
            style={{
              backgroundColor: 'rgba(255,255,255,0.5)',
              backdropFilter: 'blur(8px)',
              WebkitBackdropFilter: 'blur(8px)'
          ...
          >
            {isLoggedIn && (
              <div className="flex items-start gap-2 md:gap-3 relative z-20">
                {/* 自訂毛玻璃勾選框 */}
                <label htmlFor="anonymous-checkbox" style={{ position: 'relative', display: 'inline-block', width: '18px', height: '18px', zIndex: 30 }}>
                  <input
                    type="checkbox"
                ...
                <label htmlFor="anonymous-checkbox" className="text-[#1694da] text-xs font-normal leading-5 text-left" style={{ cursor: 'pointer', zIndex: 30 }}>
