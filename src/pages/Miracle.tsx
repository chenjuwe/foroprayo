...
import { useMiraclePosts, useCreateMiraclePost, useDeleteMiraclePost } from '@/hooks/useMiraclePosts';
import PrayerPost from '@/components/PrayerPost';
import { PrayerPostSkeletonList } from '@/components/ui/skeleton';
import type { MiraclePost, Prayer } from '@/types/prayer';

export default function Miracle() {
  const { user, isLoggedIn, avatarUrl } = useFirebaseAvatar();
  const [isGuestMode, setIsGuestMode] = useState(false);
  const createMiraclePostMutation = useCreateMiraclePost();
  const deleteMiraclePostMutation = useDeleteMiraclePost();
  ...
  const handlePostDeleted = useCallback((deletedPostId: string) => {
    deleteMiraclePostMutation.mutate(deletedPostId);
  }, [deleteMiraclePostMutation]);
  ...
  const handlePrayerSubmit = async (e: React.FormEvent) => {
    ...
    createMiraclePostMutation.mutate({
      content: prayerText,
          ...
          <div className="flex flex-col max-w-[358px] mx-auto">
            <div className="bg-white w-full px-4 pt-4 pb-[12px] shadow-sm">
              <PrayerForm
                ...
                isSubmitting={createMiraclePostMutation.isPending}
                placeholder="分享你的神蹟奇事..."
                rows={1}
