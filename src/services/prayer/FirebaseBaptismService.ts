...
import type { BaptismPost, CreateBaptismPostRequest } from '@/types/prayer';
import { getUnifiedUserName } from '@/lib/getUnifiedUserName';

/**
 * Service for managing Baptism posts in Firebase.
 */
export class FirebaseBaptismService extends BaseService {
  private readonly COLLECTION_NAME = 'baptism';

  constructor() {
    super('FirebaseBaptismService');
  }

  private convertDocToPost(doc: DocumentData): BaptismPost {
    const data = doc.data();
  ...
  async getAllPosts(): Promise<BaptismPost[]> {
  ...
  async getPostById(id: string): Promise<BaptismPost | null> {
  ...
  async createPost(post: CreateBaptismPostRequest): Promise<BaptismPost> {
