...
import type { MiraclePost, CreateMiraclePostRequest } from '@/types/prayer';
import { getUnifiedUserName } from '@/lib/getUnifiedUserName';

/**
 * Service for managing Miracle posts in Firebase.
 */
export class FirebaseMiracleService extends BaseService {
  private readonly COLLECTION_NAME = 'miracle';

  constructor() {
    super('FirebaseMiracleService');
  }

  private convertDocToPost(doc: DocumentData): MiraclePost {
    const data = doc.data();
  ...
  async getAllPosts(): Promise<MiraclePost[]> {
  ...
  async getPostById(id: string): Promise<MiraclePost | null> {
  ...
  async createPost(post: CreateMiraclePostRequest): Promise<MiraclePost> {
